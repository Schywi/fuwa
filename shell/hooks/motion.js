	...
	*/
	...
	var mermaidDefinitions = {
		frontend: joinDiagram([
			...
			'    direction LR',
			'    HTMX["HTMX 1.9<br/>AJAX swaps"]:::browser',
			'    PV["petite-vue 0.4<br/>reactive state"]:::browser',
			'    Editor["editor.js<br/>CodeMirror 6"]:::hook',
			'    Terminal["terminal.js<br/>xterm.js"]:::hook',
			'    WS["workspace.js<br/>popovers · panels"]:::hook',
			'    Preview["preview.js<br/>orchestrator"]:::hook',
			'    PB["preview-browser.js<br/>tenant iframe · relay"]:::hook',
			'    Obs["observability.js<br/>SSE traces"]:::hook',
			'    Tmux["tmux.js<br/>container logs"]:::hook',
			'  end',
			'  subgraph Worker["Web Worker"]',
			'    RS["runtime-session.js<br/>lifecycle · debounce"]:::worker',
			'    RW["runtime-worker.js<br/>Wasmoon Lua 5.4"]:::worker',
			'  end',
			'  subgraph Tenant["Tenant iframe"]',
			'    TR["tenant-runtime.js<br/>TenantXHR · swapHtml"]:::tenant',
			'  end',
		...
		backend: joinDiagram([
			...
			'---',
			'graph LR',
			'  classDef python fill:#1a1b26,stroke:#e0af68,stroke-width:2px,color:#c0caf5',
			...
			'    direction LR',
			'    DevServer["dev-server.py<br/>HTTP · file watcher"]:::python',
			'    CL["container-logs.py<br/>docker logs mux"]:::python',
			'  end',
			'  subgraph CGI["Lua CGI · fuwa-dev.lua"]',
			'    CGI["fuwa-dev.lua<br/>dispatch · static files"]:::lua',
			'  end',
			'  subgraph Compiler["Compiler Pipeline"]',
			'    direction LR',
			'    PW["package_web.build"]:::compiler',
			'    Mod["modules.lua<br/>.fuwa → .lua"]:::compiler',
			'    Act["actions.lua<br/>sugar"]:::compiler',
			'    ViewC["view.lua<br/>includes"]:::compiler',
			'    Boot["bootstrap.lua<br/>main.lua gen"]:::compiler',
			'  end',
			'  subgraph Runtime["Runtime stdlib"]',
			'    direction LR',
			'    Web["web.lua<br/>router"]:::runtime',
			'    View["view.lua<br/>SSR engine"]:::runtime',
			'    DB["db.lua<br/>provider facade"]:::runtime',
			'    Trace["trace.lua<br/>spans"]:::runtime',
			'  end',
			'  subgraph Host["Host Capabilities"]',
			'    Cap["capabilities.lua<br/>file I/O · compile"]:::runtime',
			'    Dash["dashboard.lua<br/>file tree · terminal"]:::runtime',
			'  end',
			'  DevServer -->|"stdin→stdout"| CGI',
			'  CGI --> PW',
			'  PW --> Mod --> Act',
			'  PW --> ViewC --> Boot',
			'  CGI --> Web --> View',
			'  CGI --> Cap',
			'  CGI --> Dash',
			'  Dash -->|"data → template"| View',
			'  View -->|"HTML"| output["HTTP Response"]:::output'
		]),
		infra: joinDiagram([
