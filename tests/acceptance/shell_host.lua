...
return function(t)
	...
	t.test("shell response exposes the dashboard workspace and shell hook", function()
		...
		t.truthy(response.body:find("build fuwa-gomen", 1, true) ~= nil, "expected payload listing")
		...
		t.truthy(response.body:find('data-select', 1, true) ~= nil, "expected payload select control")
