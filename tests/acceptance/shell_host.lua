...
return function(t)
	...
	t.test("shell response exposes the dashboard workspace and shell hook", function()
		...
		t.truthy(response.body:find("pipeline smoke test", 1, true) ~= nil, "expected shell title")
		t.truthy(response.body:find("documents · build ok", 1, true) ~= nil, "expected compile summary")
		...
		t.truthy(response.body:find('data-switch-select', 1, true) ~= nil, "expected payload switcher")
		t.truthy(response.body:find('cdn.jsdelivr.net/npm/iconify-icon', 1, true) ~= nil, "expected icon CDN loader")
	end)
