...
return function(t)
	...
	t.test("shell response exposes the dashboard workspace and shell hook", function()
		...
		t.truthy(response.body:find("pipeline smoke test", 1, true) ~= nil, "expected donor shell title")
		t.truthy(response.body:find('data-preview-stage', 1, true) ~= nil, "expected browser runtime stage")
		t.falsy(response.body:find('src="/payload/fuwa-gomen/"', 1, true) ~= nil, "expected no route-backed iframe in the default shell")
		...
		t.truthy(response.body:find('hx-post="/switch"', 1, true) ~= nil, "expected payload switch form")
		t.truthy(response.body:find('id="asset-switcher"', 1, true) ~= nil, "expected donor-like asset select")
