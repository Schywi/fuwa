...
t.test("fuwa-gomen uses the full-bleed phone shell root contract", function()
	...
	t.contains(layout, 'class="gomen-shell"', "expected shell card wrapper")
	t.contains(layout, "script defer src=\"browser.js\"", "expected local browser asset tag")
	t.contains(view, 'class="fg-root"', "expected semantic gomen root")
...
t.test("fuwa-gomen browser bundle skips tenant auto-mounting in the iframe runtime", function()
	...
	t.contains(browser_js, "window.PetiteVue.createApp().mount(scope)", "expected direct-route mount")
	t.contains(browser_js, "new XMLHttpRequest()", "expected XHR persistence helper")
	...
	t.contains(browser_js, "/vendor/gsap/gsap-3.15.0.min.js", "expected local GSAP path")
end)
