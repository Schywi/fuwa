...
t.test("browser worker contract exposes the bootstrap message set", function()
	...
	t.eq(#types, 9, "expected nine message types")
	...
	t.eq(types[8], "trace", "expected trace message")
	t.eq(types[9], "done", "expected done message")
...
t.test("browser runtime bootstrap stays a thin seam", function()
	...
	t.eq(#mount.messages, 9, "expected worker contract in the mount descriptor")
	t.eq(mount.messages[1], "boot", "expected contract message order")
	t.eq(mount.messages[8], "trace", "expected contract message order")
	t.eq(mount.messages[9], "done", "expected contract message order")
...
t.test("runtime bridge rebinds browser URLs through the payload base", function()
	...
	t.contains(tenant, "function isHostAssetPath(path)", "expected host asset path guard")
	t.contains(tenant, "'/plugins'", "expected plugin assets to stay absolute")
	t.contains(tenant, "'/__dev'", "expected dev routes to stay absolute")
end)
