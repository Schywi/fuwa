...
t.test("browser runtime uses the published bundle plus in-memory edits", function()
	local preview_browser = read_file("shell/hooks/preview-browser.js")
	t.falsy(preview_browser:find("draft: true", 1, true) ~= nil, "expected browser mode to skip draft bundles")
	t.contains(preview_browser, "bundleUrl: stage.getAttribute('data-bundle-url')", "expected bundle url wiring")
	t.contains(preview_browser, "session.updateCode(path, contents)", "expected in-memory live updates")
	t.contains(preview_browser, "session.exportSnapshot()", "expected snapshot export for deploy")
	t.contains(preview_browser, "fetch('/__dev/deploy'", "expected deploy api call")
	t.contains(preview_browser, "window.location.assign(result.payload.url)", "expected deploy redirect")
	t.contains(preview_browser, "updateCode(path, contents)", "expected direct browser update forwarding")
end)
...
t.test("fuwa-gomen uses the full-bleed phone shell root contract", function()
	...
	t.contains(layout, '&unsafe doctype', "expected full document doctype binding")
	t.contains(layout, 'class="phone-screen"', "expected phone-screen shell contract")
	t.contains(layout, "script defer src=\"browser.js\"", "expected local browser asset tag")
	t.contains(view, 'views/fragments/gomen_root.fuwa', "expected shared gomen root fragment")
end)
...
t.test("fuwa-gomen browser bundle skips tenant auto-mounting in the iframe runtime", function()
	...
	t.contains(browser_js, "htmx:beforeRequest", "expected htmx-driven animation hook")
	t.contains(browser_js, "data-price", "expected server-owned item metadata")
	t.contains(browser_js, "document.getElementById('gomen')", "expected root lookup helper")
	t.falsy(browser_js:find("fetch%(", 1, false) ~= nil, "expected no fetch mutations")
	t.falsy(browser_js:find("new XMLHttpRequest()", 1, true) ~= nil, "expected no XHR persistence helper")
	t.falsy(browser_js:find("function render%(", 1, false) ~= nil, "expected no manual render loop")
	t.falsy(browser_js:find("renderReceipt", 1, true) ~= nil, "expected no receipt painter helper")
	t.falsy(browser_js:find("setFace", 1, true) ~= nil, "expected no face painter helper")
	t.falsy(browser_js:find("setBubble", 1, true) ~= nil, "expected no bubble painter helper")
	t.falsy(browser_js:find("setBar", 1, true) ~= nil, "expected no bar painter helper")
end)
...
t.test("preview controller is browser-only", function()
	...
	local server_driver_pos = preview:find("server_driver", 1, true)
	local runtime_mode_pos = preview:find("runtime_mode", 1, true)
	local draft_timer_pos = preview:find("draft_timer", 1, true)
	local pending_drafts_pos = preview:find("pending_drafts", 1, true)
	local refresh_token_pos = preview:find("refreshToken", 1, true)

	t.truthy(listener_pos, "expected editor change listener")
	t.falsy(server_driver_pos ~= nil, "expected no server driver in the default preview controller")
	t.falsy(runtime_mode_pos ~= nil, "expected no runtime mode split in the default preview controller")
	t.falsy(draft_timer_pos ~= nil, "expected no draft debounce timer")
	t.falsy(pending_drafts_pos ~= nil, "expected no pending draft queue")
	t.falsy(refresh_token_pos ~= nil, "expected no refresh token plumbing")
	t.contains(preview, "mountBrowserDriver()", "expected browser mount lifecycle")
	t.contains(preview, "browser_driver.updateCode(path, contents)", "expected direct browser update calls")
	t.contains(preview, "[data-deploy-trigger]", "expected dedicated deploy trigger")
	t.contains(preview, "browser_driver.deploy()", "expected browser deploy action")
	t.falsy(browser_index:find("window%.FuwaShellPreview", 1) ~= nil, "expected no browser preview global bridge")
end)

t.test("home workspace renders a plain deploy button instead of an htmx action", function()
	local home = read_file("shell/views/fragments/home.fuwa")
	t.contains(home, 'data-deploy-trigger', "expected deploy trigger attribute")
	t.contains(home, '<button type="button" class="grafana-trigger" data-deploy-trigger', "expected plain button deploy control")
	t.falsy(home:find('href="/__dev/deploy"', 1, true) ~= nil, "expected no deploy anchor")
	t.falsy(home:find('hx%-post="/__dev/deploy"', 1, false) ~= nil, "expected no htmx deploy post")
end)
...
t.test("payload browser bootstraps wait for vendor libraries", function()
	...
	t.falsy(current_browser:find("window%.FuwaCurrent", 1) ~= nil, "expected no payload global marker")
end)
