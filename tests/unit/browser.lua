...
t.test("fuwa-gomen uses the donor root and DOM-ref contract", function()
	...
	t.contains(view, 'v-scope="FuwaGomen.createScope()"', "expected donor scope binding")
	t.contains(view, 'data-balance="&balance"', "expected server-seeded balance dataset")
	t.contains(view, 'data-spent="&spent"', "expected server-seeded spent dataset")
	t.contains(view, 'data-pokes="&pokes"', "expected server-seeded pokes dataset")
	t.contains(view, 'data-ref="fill"', "expected imperative bar ref")
	t.contains(view, 'data-ref="receipt"', "expected imperative receipt ref")
end)
...
t.test("fuwa-gomen browser bundle skips tenant auto-mounting in the iframe runtime", function()
	...
	t.contains(browser_js, "initialState:", "expected donor bootstrap state")
	t.contains(browser_js, "script.src = '/vendor/gsap/gsap-3.15.0.min.js';", "expected vendored gsap seam")
	t.contains(browser_js, "const state = { value: shownTotal };", "expected donor imperative receipt total animation")
	t.contains(browser_js, "boot.root.setAttribute('data-mood', mood.moodKey);", "expected donor mood painting")
