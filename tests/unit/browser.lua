...
t.test("browser runtime uses the published bundle plus in-memory edits", function()
	...
	t.contains(preview_browser, "session.updateSources(edits)", "expected in-memory live updates")
end)
...
t.test("editor changes trigger browser live updates before draft persistence", function()
	...
	local browser_branch_pos = preview:find("if (runtime_mode === 'browser') {", listener_pos or 1, true)
	local live_update_pos = preview:find("liveUpdateActiveBrowserDriver({", browser_branch_pos or 1, true)
	local draft_timer_pos = preview:find("draft_timer = setTimeout(flushDrafts, DRAFT_DEBOUNCE_MS);", listener_pos or 1, true)
	local browser_flush_guard = preview:find("if (runtime_mode === 'browser') {", preview:find("function flushDrafts()", 1, true) or 1, true)

	t.truthy(listener_pos, "expected editor change listener")
	t.truthy(browser_branch_pos, "expected explicit browser-mode branch")
	t.truthy(live_update_pos, "expected direct browser live update call")
	t.truthy(draft_timer_pos, "expected draft debounce timer")
	t.truthy(browser_flush_guard, "expected browser flush guard")
