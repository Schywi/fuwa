...
t.test("openresty ai manifest exposes the planned model contract", function()
	...
	t.eq(payload.models[1].available, false, "expected retrieval model to report missing local artifacts on this branch")
	t.eq(payload.models[2].available, false, "expected generation model to report missing local artifacts on this branch")
	t.eq(payload.models[1].artifacts[1].available, false, "expected artifact availability metadata")
	t.truthy(#payload.models[1].missing_artifacts > 0, "expected missing artifact list")
	...
	t.truthy(model_catalog.resolve_artifact_path("/ai/models/model2vec-potion-base-8m/model.onnx") ~= nil, "expected artifact path resolver for known manifest artifact")
	t.eq(model_catalog.resolve_artifact_path("/ai/models/../../escape"), nil, "expected artifact path resolver to reject traversal")
	t.truthy(response.body:find('"id":"model2vec%-potion%-base%-8m"') ~= nil, "expected manifest json body")
	t.truthy(response.body:find('"available":false') ~= nil, "expected manifest availability metadata")
