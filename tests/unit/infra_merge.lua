...
t.test("root nginx keeps dev handlers and proxies dashboard routes", function()
	...
	t.contains(nginx, "location = /uptrace-local/ {", "expected Uptrace no-typing helper route")
	t.contains(nginx, "POST /internal/v1/users/login HTTP/1.1", "expected helper route to use Uptrace's normal login endpoint")
	t.contains(nginx, 'return ngx.redirect("/dash/uptrace/")', "expected helper route to redirect into the proxied Uptrace dashboard")
...
t.test("shell tmux pane points at the real app container", function()
	...
	t.contains(home, "window.open('/uptrace-local/'", "expected the Uptrace button to use the no-typing helper route")
