...
t.test("shell tmux pane points at the real app container", function()
	...
	t.contains(home, '@click="openUptrace()"', "expected Uptrace button to use its own independent handler")
	...
	t.contains(home, 'src="/dash/uptrace/"', "expected Uptrace iframe route")
	t.not_contains(home, ':src="dashboardSrc"', "expected no shared iframe source state between SigNoz and Uptrace")
