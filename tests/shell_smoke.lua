...
assert_true(type(html) == "string" and html:find("pipeline smoke test", 1, true) ~= nil, "shell should render donor host branding")
...
assert_true(html:find('src="/payload/fuwa-gomen/"', 1, true) == nil, "shell should not mount the route-backed tenant iframe by default")
...
assert_true(html:find('hx-post="/switch"', 1, true) ~= nil, "shell should expose a payload switch form")
assert_true(html:find('id="asset-switcher"', 1, true) ~= nil, "shell should expose the donor asset select")
...
assert_true(html:find('hx-get="/inspect/fuwa-gomen?file=app.fuwa"', 1, true) ~= nil, "shell should expose donor file inspection links")
...
assert_true(html:find('data-file-path="app.fuwa"', 1, true) ~= nil, "shell should pass the donor entry file path")
...
assert_true(html:find('data-terminal-session="fuwa-gomen"', 1, true) ~= nil, "shell should pass the donor terminal session")
