...
assert_true(observability_js:find("mount: mount, unmount: unmount, refresh: refresh", 1, true) ~= nil, "shell should expose observability mount controls against the current root")
...
assert_true(html:find("documents · build ok", 1, true) ~= nil, "shell should render the compile summary")
...
assert_true(html:find('data-switch-select', 1, true) ~= nil, "shell should expose a payload switch dropdown")
assert_true(html:find('data-switch-current="/switch/fuwa-gomen"', 1, true) ~= nil, "shell should expose the current switch route")
