...
local function test_http_request()
	...
	assert_true(output:find("pipeline smoke test", 1, true) ~= nil, "expected rendered shell")
	...
	assert_true(output:find('/shell/hooks/browser/index.js', 1, true) ~= nil, "expected browser ESM entrypoint")
	assert_true(output:find('/shell/hooks/editor.js', 1, true) == nil, "expected no individual editor script tag")
	assert_true(output:find('/shell/hooks/terminal.js', 1, true) == nil, "expected no individual terminal script tag")
...
local function test_response_builder()
	...
	assert_true(response.body:find('<div class="ide-shell" v-scope="workspace"', 1, true) ~= nil, "expected petite-vue on the stable shell parent")
	...
	assert_true(response.body:find('/shell/hooks/browser/index.js', 1, true) ~= nil, "expected browser ESM entrypoint")
	assert_true(response.body:find('/shell/hooks/editor.js', 1, true) == nil, "expected no individual editor script tag")
	assert_true(response.body:find('/shell/hooks/terminal.js', 1, true) == nil, "expected no individual terminal script tag")
...
local function test_shell_switch_route()
	...
	assert_true(response.body:find('shell-command-trigger', 1, true) ~= nil, "expected single command launcher")
	assert_true(response.body:find('data-popover="files"', 1, true) ~= nil, "expected single file palette")
...
local function test_raw_asset_requests()
	...
	assert_true(editor_js:find("window.FuwaShellEditor", 1, true) == nil, "expected editor module to stay free of compatibility globals")
	...
	assert_true(browser_index_js:find("import '../observability.js';", 1, true) ~= nil, "expected entrypoint to use direct ESM module imports")
	assert_true(browser_index_js:find("window.FuwaPreviewBrowserDriver", 1, true) == nil, "expected entrypoint to avoid preview browser globals")
	assert_true(browser_index_js:find("window.FuwaShellEditor", 1, true) == nil, "expected entrypoint to avoid editor globals")
	assert_true(browser_index_js:find("window.FuwaShellTerminal", 1, true) == nil, "expected entrypoint to avoid terminal globals")
	assert_true(browser_index_js:find("window.FuwaShellWorkspace", 1, true) == nil, "expected entrypoint to avoid workspace globals")
	assert_true(browser_index_js:find("window.FuwaShellMotion", 1, true) == nil, "expected entrypoint to avoid motion globals")
	assert_true(browser_index_js:find("window.FuwaShellPreview", 1, true) == nil, "expected entrypoint to avoid preview globals")
	assert_true(browser_index_js:find("import '../editor.js';", 1, true) ~= nil, "expected entrypoint to load editor ESM directly")
	assert_true(browser_index_js:find("import '../terminal.js';", 1, true) ~= nil, "expected entrypoint to load terminal ESM directly")
	assert_true(browser_index_js:find("import '../workspace.js';", 1, true) ~= nil, "expected entrypoint to load workspace ESM directly")
	assert_true(browser_index_js:find("import '../preview.js';", 1, true) ~= nil, "expected entrypoint to load preview ESM directly")
	...
	assert_true(workspace_js:find("openPalette", 1, true) ~= nil, "expected command palette entrypoint")
	assert_true(workspace_js:find("event.metaKey || event.ctrlKey", 1, true) ~= nil, "expected cmd-k shortcut")
	...
	assert_true(browser_index_js:find("HTTP/1.1 200 OK", 1, true) ~= nil, "expected browser entrypoint to respond")
	assert_true(browser_index_js:find("window.FuwaRuntimeSession", 1, true) == nil, "expected browser entrypoint to avoid the runtime session global")
	assert_true(browser_index_js:find("window.FuwaPreviewBrowserDriver", 1, true) == nil, "expected browser entrypoint to avoid the preview-browser global")
	assert_true(browser_index_js:find("window.FuwaShellTmux", 1, true) == nil, "expected browser entrypoint to avoid the tmux global")
	assert_true(browser_index_js:find("window.FuwaShellCursor", 1, true) == nil, "expected browser entrypoint to avoid the cursor global")
	...
	assert_true(terminal_js:find("window.FuwaShellTerminal", 1, true) == nil, "expected terminal module to stay free of compatibility globals")
...
local function test_shell_inspect_fragment()
	...
	assert_true(response.body:find('data-popover="files"', 1, true) ~= nil, "expected command palette popover")
	assert_true(response.body:find('shell-command-trigger', 1, true) ~= nil, "expected single launcher trigger")
