...
local function test_raw_asset_requests()
	...
	assert_true(browser_index_js:find("import {", 1, true) ~= nil, "expected entrypoint to use ESM imports")
	assert_true(browser_index_js:find("from '../editor.js';", 1, true) ~= nil, "expected entrypoint to import editor helpers")
	...
	assert_true(browser_index_js:find("window.FuwaShellEditor", 1, true) ~= nil, "expected entrypoint to expose the editor compatibility global")
	assert_true(browser_index_js:find("window.FuwaShellTerminal", 1, true) ~= nil, "expected entrypoint to expose the terminal compatibility global")
	assert_true(browser_index_js:find("window.FuwaShellWorkspace", 1, true) ~= nil, "expected entrypoint to expose the workspace compatibility global")
	assert_true(browser_index_js:find("window.FuwaShellMotion", 1, true) ~= nil, "expected entrypoint to expose the motion compatibility global")
	assert_true(browser_index_js:find("window.FuwaShellPreview", 1, true) ~= nil, "expected entrypoint to expose the preview compatibility global")
