...
local function test_http_request()
	...
	assert_true(output:find("pipeline smoke test", 1, true) ~= nil, "expected rendered donor shell")
	assert_true(output:find('data-preview-stage', 1, true) ~= nil, "expected browser runtime stage")
	assert_true(output:find('src="/payload/fuwa-gomen/"', 1, true) == nil, "expected no route-backed iframe")
	...
	assert_true(output:find('hx-post="/switch"', 1, true) ~= nil, "expected shell switch form")
	assert_true(output:find('id="asset-switcher"', 1, true) ~= nil, "expected donor asset switcher")
...
local function test_response_builder()
	...
	assert_true(response.body:find("pipeline smoke test", 1, true) ~= nil, "expected donor shell response")
	...
	assert_true(response.body:find('src="/payload/fuwa-gomen/"', 1, true) == nil, "expected no route-backed iframe")
	...
	assert_true(response.body:find('hx-post="/switch"', 1, true) ~= nil, "expected switch form")
	assert_true(response.body:find('id="asset-switcher"', 1, true) ~= nil, "expected donor asset select")
