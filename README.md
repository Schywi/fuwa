...
## Current Lean Runtime

The install/runtime notes above are the historical shell description.
The current tree now includes a direct Node dev server and a root
`package.json`, so the real local flow is:
...
- `npm run dev` starts `src/dev.ts`, which serves the Fuwa payload,
  `/browser.js`, `/vendor/gsap.min.js`, and `/vendor/petite-vue.js`
  without Vite.
- `npm test` runs the compiler/runtime smoke tests against the
  `fuwa-gomen` payload.
- `npm run check` runs the TypeScript checker with no emit.

Current root dependencies are intentionally lean:

- **Runtime**: `wasmoon`, `gsap`, `petite-vue`, `chokidar`
- **Tooling**: `tsx`, `typescript`, `@types/node`
- **Platform**: Node.js 20+ and a browser with WebAssembly support

The browser shell still uses the same jungle-style GSAP loading pattern:
`window.__FUWA_IDE_TENANT_BASE__` points at `/vendor`, and the payload
bootstrap loads `gsap.min.js` from that local base instead of a CDN.
